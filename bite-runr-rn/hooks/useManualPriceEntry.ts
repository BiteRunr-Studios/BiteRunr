    ...
    useEffect(() => {
        ...
        AsyncStorage.getItem(draftKey)
            .then((raw) => {
                ...
                setHasDraft(true);
                setPrices(new Map(Object.entries(draft.prices).map(
                    ([k, v]) => [k, v as number | null],
                )));
            })
            .catch((e) =>
                console.error("Failed to check manual prices draft:", e),
            );
    }, [draftKey]);
    ...
    const saveAll = useCallback(async () => {
        ...
        try {
            ...
            if (
                ...
                setError(
                    "We couldn't save your prices. Please try again.",
                );
            }
            setState("error");
        }
