...
type Tab = "friends" | "requests" | "search" | "squads";

const SQUAD_COLORS: { key: string; bg: string; deep: string; icon: keyof typeof icons }[] = [
...
function TabPill({ label, active, badge, onPress }: { label: string; active: boolean; badge?: number; onPress: () => void }) {
    return (
        ...
        >
            <Text style={{ fontFamily: FONTS.display.bold, fontSize: 13, color: active ? "#fff" : COLORS.ink2 }}>
                {label}
            </Text>
            {!!badge && badge > 0 && (
                <View style={{ minWidth: 18, height: 18, borderRadius: 9, backgroundColor: COLORS.coral, alignItems: "center", justifyContent: "center", paddingHorizontal: 4 }}>
...
function CreateSquadSheet({
    ...
    visible: boolean;
    onClose: () => void;
    friends: { id: Id<"users">; firstName: string; lastName: string; avatarUrl?: string }[];
}) {
    ...
    const colorDef = SQUAD_COLORS.find((c) => c.key === selectedColor) ?? SQUAD_COLORS[0];
            ...
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
                <View style={{ flex: 1, backgroundColor: COLORS.paper }}>
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 18, paddingVertical: 14, gap: 12 }}>
                        <View style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: colorDef.bg, alignItems: "center", justifyContent: "center" }}>
                            <Icon name={colorDef.icon} size={20} color="#fff" />
                        </View>
                        <View style={{ flex: 1 }}>
                            <Text style={{ fontFamily: FONTS.display.bold, fontSize: 22, letterSpacing: -0.4, color: COLORS.ink }}>
                                Create a <Text style={{ color: COLORS.orange, fontStyle: "italic" }}>squad</Text>
                            </Text>
                            <Text style={{ fontSize: 12, color: COLORS.ink3, marginTop: 1 }}>Reusable group for fast runs & splits</Text>
                        </View>
                        <Pressable onPress={handleClose} style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: COLORS.paper2, borderWidth: 1, borderColor: COLORS.line, alignItems: "center", justifyContent: "center" }}>
                    ...
                    <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
                        {/* Name */}
                        <Text style={{ fontFamily: FONTS.mono.semibold, fontSize: 10, letterSpacing: 1.2, textTransform: "uppercase", color: COLORS.ink3, marginBottom: 8 }}>
                            Squad name
                        </Text>
                        <TextInput
                            value={name}
                        ...
                        {/* Color picker */}
                        <Text style={{ fontFamily: FONTS.mono.semibold, fontSize: 10, letterSpacing: 1.2, textTransform: "uppercase", color: COLORS.ink3, marginBottom: 10 }}>
                            Pick a vibe
                        </Text>
                        ...
                        {friends.length === 0 ? (
                            <Text style={{ fontSize: 13, color: COLORS.ink3, marginBottom: 20 }}>Add some friends first to include them in a squad.</Text>
                        ) : (
                            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 24 }}>
                                {friends.map((f) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <Avatar name={fullName} avatarUrl={f.avatarUrl} size={24} />
                                            <Text style={{ fontFamily: FONTS.display.semibold, fontSize: 13, color: isSelected ? "#fff" : COLORS.ink }}>
                                                {f.firstName}
                                            </Text>
...
function FriendsTab({ pendingCount }: { pendingCount: number }) {
    ...
    const handleRemove = (id: Id<"users">, name: string) => {
        ...
        Alert.alert("Remove Friend", `Remove ${name} from your friends?`, [
            { text: "Cancel", style: "cancel" },
            {
                ...
                onPress: async () => {
                    setRemovingId(id);
                    try { await removeFriend({ friendId: id }); }
                    catch (e: any) { Alert.alert("Error", e?.message ?? "Failed to remove friend"); }
    ...
    if (friends === undefined) {
        return (
            <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 32, gap: 10 }}>
                <Skeleton>
                    {[1, 2, 3, 4].map((i) => <SkeletonBlock key={i} width="100%" height={68} rounded="rounded-2xl" />)}
                </Skeleton>
            </ScrollView>
        );
    }
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 32 }} showsVerticalScrollIndicator={false}>
            ...
            {pendingCount > 0 && (
                <Animated.View entering={FadeIn.duration(300)}>
                    <Pressable
                        onPress={() => { /* parent handles tab switch — pass via prop below */ }}
                        style={{ flexDirection: "row", alignItems: "center", gap: 10, padding: 12, borderRadius: 16, backgroundColor: COLORS.orangeTint, borderWidth: 1, borderColor: "rgba(255,106,31,0.2)", marginBottom: 12 }}
                    >
                        <View style={{ flexDirection: "row" }}>
                            {(requests ?? []).slice(0, 3).map((r, j) => (
                                <View key={r.id} style={{ marginLeft: j > 0 ? -10 : 0, borderRadius: 999, shadowColor: COLORS.orangeTint, shadowRadius: 0 }}>
                                    <Avatar name={`${r.sender?.firstName} ${r.sender?.lastName}`} avatarUrl={r.sender?.avatarUrl} size={28} />
                                </View>
                            ))}
                        </View>
                        <Text style={{ fontFamily: FONTS.display.semibold, fontSize: 13, color: COLORS.orangeDeep, flex: 1 }}>
            ...
            {/* Search bar */}
            {(friends?.length ?? 0) > 3 && (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "#fff", borderRadius: 14, borderWidth: 1, borderColor: COLORS.line, paddingHorizontal: 12, height: 42, marginBottom: 12 }}>
                    ...
                    <TextInput
                        ...
                        placeholderTextColor={COLORS.ink3}
                        style={{ flex: 1, fontFamily: FONTS.display.regular, fontSize: 14, color: COLORS.ink }}
                    />
                    {search.length > 0 && (
            ...
            {friends.length === 0 ? (
                <View style={{ alignItems: "center", paddingTop: 48, paddingHorizontal: 24 }}>
                    <View style={{ width: 64, height: 64, borderRadius: 20, backgroundColor: COLORS.paper2, alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
                        <Icon name="Users" size={28} color={COLORS.ink3} />
                    </View>
                    <Text style={{ fontFamily: FONTS.display.bold, fontSize: 17, color: COLORS.ink }}>No friends yet</Text>
                    <Text style={{ fontSize: 13, color: COLORS.ink3, textAlign: "center", marginTop: 6, lineHeight: 20 }}>
                        Search for people to add them as friends and start ordering together
                    </Text>
                </View>
            ) : filtered.length === 0 ? (
                <View style={{ alignItems: "center", paddingTop: 32 }}>
                    <Text style={{ fontSize: 13, color: COLORS.ink3 }}>No friends matching "{search}"</Text>
                </View>
            ) : (
                <View style={{ gap: 8 }}>
                                ...
                                <AnimatedPressable scale={0.98} onLongPress={() => handleRemove(friend.id, `${friend.firstName} ${friend.lastName}`)} style={{ flexDirection: "row", alignItems: "center", gap: 12, padding: 14, backgroundColor: COLORS.orangeTint, borderRadius: 18, borderWidth: 1, borderColor: "rgba(255,106,31,0.2)" }}>
                                    ...
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <View style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 2 }}>
                                            ...
                                            <Text style={{ fontSize: 12, color: COLORS.mint, fontFamily: FONTS.display.semibold }}>Friends</Text>
                                        </View>
                                    </View>
                                    <Pressable
                                        onPress={() => handleRemove(friend.id, fullName)}
                                        disabled={removingId === friend.id}
                                        style={{ width: 38, height: 38, borderRadius: 999, backgroundColor: "#fff", borderWidth: 1, borderColor: COLORS.line, alignItems: "center", justifyContent: "center" }}
                                    >
...
function RequestsTab() {
    ...
    if (requests === undefined) {
        return (
            <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 32, gap: 10 }}>
                <Skeleton>
                    {[1, 2].map((i) => <SkeletonBlock key={i} width="100%" height={100} rounded="rounded-2xl" />)}
                </Skeleton>
            </ScrollView>
        );
    }
    ...
    if (requests.length === 0) {
        return (
            <View style={{ alignItems: "center", paddingTop: 48, paddingHorizontal: 24 }}>
                ...
                <Text style={{ fontFamily: FONTS.display.bold, fontSize: 17, color: COLORS.ink }}>No pending requests</Text>
                <Text style={{ fontSize: 13, color: COLORS.ink3, textAlign: "center", marginTop: 6, lineHeight: 20 }}>
                            ...
                            <View style={{ padding: 14, borderRadius: 18, backgroundColor: COLORS.orangeTint, borderWidth: 1, borderColor: "rgba(255,106,31,0.2)" }}>
                                <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                                    <View style={{ position: "relative" }}>
                                        <Avatar name={fullName} avatarUrl={req.sender.avatarUrl} size={48} />
                                        <View style={{ position: "absolute", bottom: -2, right: -2, width: 20, height: 20, borderRadius: 10, backgroundColor: COLORS.orange, alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: COLORS.orangeTint }}>
...
function SearchTab() {
    ...
    const searchResults = useQuery(api.friends.searchUsers, query.length >= 2 ? { query } : "skip");
    ...
    return (
        <View style={{ flex: 1, paddingHorizontal: 18 }}>
            {/* Search input */}
            <View style={{ flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "#fff", borderRadius: 14, borderWidth: 1, borderColor: COLORS.line, paddingHorizontal: 12, height: 46, marginBottom: 16 }}>
            ...
            {query.length < 2 && (
                <View style={{ alignItems: "center", paddingTop: 40 }}>
                    ...
                    <Text style={{ fontFamily: FONTS.display.bold, fontSize: 17, color: COLORS.ink }}>Find Friends</Text>
                    <Text style={{ fontSize: 13, color: COLORS.ink3, textAlign: "center", marginTop: 6 }}>Enter at least 2 characters to search</Text>
                </View>
            )}
            ...
            {isSearching && (
                <Skeleton>
                    <View style={{ gap: 10 }}>
                        {[1, 2, 3].map((i) => <SkeletonBlock key={i} width="100%" height={68} rounded="rounded-2xl" />)}
                    </View>
                </Skeleton>
            )}
            ...
            {query.length >= 2 && searchResults && searchResults.length === 0 && (
                <View style={{ alignItems: "center", paddingTop: 40 }}>
                    ...
                    <Text style={{ fontFamily: FONTS.display.bold, fontSize: 17, color: COLORS.ink }}>No users found</Text>
                    <Text style={{ fontSize: 13, color: COLORS.ink3, marginTop: 6 }}>Try a different name or email</Text>
                </View>
            )}
            ...
            {searchResults && searchResults.length > 0 && (
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 32 }}>
                    {searchResults.map((user, i) => {
                        ...
                        return (
                            <Animated.View key={user.id} entering={FadeInUp.duration(250).delay(i * 40)}>
                                <View style={{ flexDirection: "row", alignItems: "center", gap: 12, padding: 12, backgroundColor: "#fff", borderRadius: 16, borderWidth: 1, borderColor: COLORS.line }}>
                                    <Avatar name={fullName} avatarUrl={user.avatarUrl} size={46} />
                                    <View style={{ flex: 1 }}>
                                        <Text style={{ fontFamily: FONTS.display.bold, fontSize: 15, color: COLORS.ink }}>{fullName}</Text>
                                        <Text style={{ fontSize: 12, color: COLORS.ink3, marginTop: 2 }} numberOfLines={1}>{user.email}</Text>
                                    </View>
                                    <TouchableOpacity
                                        onPress={() => handleAdd(user.id, fullName)}
                                        disabled={sendingTo === user.id}
                                        activeOpacity={0.8}
                                        style={{ flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, backgroundColor: COLORS.ink }}
                                    >
                                        {sendingTo === user.id
                                            ? <ActivityIndicator size="small" color="#fff" />
                                            : <><Icon name="UserPlus" size={14} color="#fff" /><Text style={{ fontFamily: FONTS.display.semibold, fontSize: 13, color: "#fff" }}>Add</Text></>}
                                    </TouchableOpacity>
                                </View>
                            </Animated.View>
                        );
...
// ─── Squads tab ────────────────────────────────────────────────────
function SquadsTab({ onCreateSquad }: { onCreateSquad: () => void }) {
    const squads = useQuery(api.squads.list);
    const removeSquad = useMutation(api.squads.remove);
    const [deletingId, setDeletingId] = useState<string | null>(null);

    const handleDelete = (id: Id<"squads">, name: string) => {
        Alert.alert("Delete Squad", `Delete "${name}"? This can't be undone.`, [
            { text: "Cancel", style: "cancel" },
            {
                text: "Delete", style: "destructive",
                onPress: async () => {
                    setDeletingId(id);
                    try { await removeSquad({ squadId: id }); }
                    catch (e: any) { Alert.alert("Error", e?.message ?? "Failed to delete squad"); }
    ...
    if (squads === undefined) {
        return (
            <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 32, gap: 10 }}>
                <Skeleton>
                    {[1, 2].map((i) => <SkeletonBlock key={i} width="100%" height={72} rounded="rounded-2xl" />)}
                </Skeleton>
            </ScrollView>
        );
    }
    ...
    return (
        <SafeAreaView edges={["top"]} style={{ flex: 1, backgroundColor: COLORS.paper }}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 18, paddingVertical: 10, gap: 10 }}>
                <Pressable
                    onPress={() => router.back()}
                    style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: COLORS.paper2, borderWidth: 1, borderColor: COLORS.line, alignItems: "center", justifyContent: "center" }}
                >
                    <Icon name="ChevronLeft" size={20} color={COLORS.ink} />
                </Pressable>
                <Text style={{ fontFamily: FONTS.display.bold, fontSize: 20, letterSpacing: -0.4, color: COLORS.ink, flex: 1 }}>Friends</Text>
                {activeTab === "squads" && (
                    <Pressable
            ...
            <View style={{ paddingHorizontal: 18, paddingBottom: 14 }}>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
                    <TabPill label={`Friends · ${friends.length}`} active={activeTab === "friends"} onPress={() => setActiveTab("friends")} />
                    <TabPill label="Requests" active={activeTab === "requests"} badge={pendingCount} onPress={() => setActiveTab("requests")} />
                    <TabPill label="Search" active={activeTab === "search"} onPress={() => setActiveTab("search")} />
                    <TabPill label="Squads" active={activeTab === "squads"} onPress={() => setActiveTab("squads")} />
                </ScrollView>
            </View>
            ...
            <View style={{ flex: 1 }}>
                ...
                {activeTab === "squads" && <SquadsTab onCreateSquad={() => setShowCreateSquad(true)} />}
            </View>
