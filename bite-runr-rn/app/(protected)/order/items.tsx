...
function mergeVoiceTranscriptSegments(committed: string, incoming: string) {
    ...
    if (normalizedIncoming.startsWith(normalizedCommitted)) return normalizedIncoming;
    if (normalizedCommitted.startsWith(normalizedIncoming)) return normalizedCommitted;
    return normalizeVoiceTranscript(`${normalizedCommitted} ${normalizedIncoming}`);
}
    ...
    const message =
        typeof error === "string"
            ...
            : typeof error === "object" && error !== null && "error" in error
              ...
              : "";
    if (message.includes("not-allowed") || message.includes("permission") || message.includes("denied")) {
        return "Microphone access is required to use voice ordering.";
    }
    if (message.includes("no-speech") || message.includes("speech-timeout") || message.includes("audio-capture")) {
...
function VoiceRippleOrb({ isListening, isProcessingVoice }: { isListening: boolean; isProcessingVoice: boolean }) {
    ...
    useEffect(() => {
        if (isListening) {
            ...
            const pairs: [typeof r1s, typeof r1o][] = [[r1s, r1o], [r2s, r2o], [r3s, r3o]];
            const timeouts = pairs.map(([sv, ov], i) =>
                setTimeout(() => {
                    ...
                    sv.value = withRepeat(
                        withSequence(
                            withTiming(1.65, { duration: 2600, easing: Easing.out(Easing.ease) }),
            ...
            return () => {
                ...
                [r1s, r2s, r3s].forEach((v) => { cancelAnimation(v); v.value = withTiming(0.6, { duration: 300 }); });
                [r1o, r2o, r3o].forEach((v) => { cancelAnimation(v); v.value = withTiming(0, { duration: 300 }); });
                cancelAnimation(coreScale);
                coreScale.value = withTiming(1, { duration: 300 });
            };
        }
        [r1s, r2s, r3s].forEach((v) => { cancelAnimation(v); v.value = withTiming(0.6, { duration: 250 }); });
        [r1o, r2o, r3o].forEach((v) => { cancelAnimation(v); v.value = withTiming(0, { duration: 250 }); });
...
export default function WriteOrder() {
    ...
    const replaceForUserLocation = useMutation(api.orderItems.replaceForUserLocation);
    ...
    const selectedLocation = useMemo(() => {
        ...
        return orderLocations.find((location) => location.id === selectedLocationId) ?? orderLocations[0];
    ...
    useEffect(() => {
        ...
        const animateTranscript = () => {
            setDisplayedVoiceTranscript((current) => {
                ...
                if (currentLength > 0 && voiceTranscript.startsWith(current) && nextChar === " ") {
                    const nextValue = voiceTranscript.slice(0, Math.min(voiceTranscript.length, currentLength + 2));
                    timeoutId = setTimeout(animateTranscript, 16);
                    return nextValue;
                }
                if (currentLength > 0 && voiceTranscript.startsWith(current) && voiceTranscript.length - currentLength > 8) {
                    const nextValue = voiceTranscript.slice(0, Math.min(voiceTranscript.length, currentLength + 3));
                    timeoutId = setTimeout(animateTranscript, 18);
                    return nextValue;
                }
                if (voiceTranscript.startsWith(current)) {
                    const nextValue = voiceTranscript.slice(0, Math.min(voiceTranscript.length, currentLength + 1));
    ...
    useEffect(() => {
        ...
        const nextItems = normalizeOrderItems(locationEntries.entries.map((entry) => entry.text));
    ...
    const processVoiceTranscript = useCallback(
        async (transcript: string) => {
            ...
            if (!normalizedTranscript) {
                setVoiceError("We didn't catch an order. Try again or type your items in.");
                return;
            }
            ...
            try {
                ...
                if (result.items.length === 0) {
                    setVoiceError("We heard you, but couldn't turn that into order items. Try again or type it in.");
                    return;
                }
                setParsedItems(result.items);
            } catch (error: any) {
                if (!isMountedRef.current) return;
                setVoiceError(error?.message ?? "We couldn't process your voice order. Please try again.");
            } finally {
    ...
    useSpeechRecognitionEvent("result", (event: any) => {
        ...
        if (event?.isFinal) {
            const nextCommittedTranscript = mergeVoiceTranscriptSegments(committedVoiceTranscriptRef.current, transcript);
        ...
        const nextTranscript = mergeVoiceTranscriptSegments(committedVoiceTranscriptRef.current, transcript);
    ...
    useSpeechRecognitionEvent("end", () => {
        setIsListening(false);
        if (shouldProcessVoiceResultRef.current && !hasProcessedVoiceResultRef.current) {
            hasProcessedVoiceResultRef.current = true;
            shouldProcessVoiceResultRef.current = false;
    ...
    const saveCurrentLocation = useCallback(
        async ({ background = false }: { background?: boolean } = {}) => {
            ...
            const normalizedCurrentItems = normalizeOrderItems([...currentItems, itemInput]);
            const normalizedLoadedItems = normalizeOrderItems(loadedItems);
            if (areOrderItemsEqual(normalizedCurrentItems, normalizedLoadedItems)) return true;
            ...
            try {
                ...
                const message = error?.message ?? "We couldn't save your order. Please try again.";
                if (isMountedRef.current) setSaveError(message);
                if (background) {
                    Toast.show({ type: "error", text1: "Couldn't save your order", text2: message, visibilityTime: 4000 });
                } else {
        ...
        [currentItems, itemInput, loadedItems, orderUserId, replaceForUserLocation, selectedLocation],
    );
    ...
    const completeOrder = useCallback(
        async ({ background = false }: { background?: boolean } = {}) => {
            ...
            try {
                ...
                const message = error?.message ?? "We couldn't finish your order. Please try again.";
                if (isMountedRef.current) setSaveError(message);
                if (background) {
                    Toast.show({ type: "error", text1: "Couldn't finish ordering", text2: message, visibilityTime: 4000 });
                } else {
    ...
    const handleLocationPress = useCallback(
        async (nextLocationId: string) => {
            if (selectedLocation?.id === nextLocationId || isSaving || isCompletingOrder) return;
            const didSave = await saveCurrentLocation();
            if (!didSave) return;
            setCurrentItems([]);
            setLoadedItems([]);
    ...
    const startVoiceOrdering = useCallback(async () => {
        if (!selectedLocation || !orderUserId) return;
        if (Platform.OS === "web") {
            Alert.alert("Voice ordering unavailable", "Voice ordering is currently available in the native app only.");
            return;
        }
    ...
    const confirmVoiceItems = useCallback(() => {
        ...
        setVoiceModalVisible(false);
        setSaveError(null);
        Toast.show({ type: "success", text1: "Order updated", text2: items.join(", "), visibilityTime: 3000 });
    }, [parsedItems]);
    ...
    return (
        <>
            ...
            <View style={styles.header}>
                <Pressable
                    onPress={() => { if (isCompletingOrder) return; router.back(); }}
                    style={styles.backBtn}
                >
                    <Icon name="ChevronLeft" size={20} color={BR.ink} />
                </Pressable>
                <BrText weight="bold" style={{ fontSize: 17 }}>Your order</BrText>
                {totalItems > 0 ? (
            ...
            <View style={{ flex: 1, backgroundColor: BR.paper }}>
                    ...
                    >
                        ...
                        <Animated.View entering={FadeInUp.duration(300)} style={{ paddingHorizontal: 2 }}>
                            ...
                            <Text style={styles.titleSub}>
                                {totalLocations === 1
                                    ? `From ${selectedLocationName} · type or dictate`
                                    : `${totalLocations} stops on this run · type or dictate`}
                            </Text>
                        </Animated.View>

                        {/* Location tabs — only shown when multi-stop */}
                        {orderLocations.length > 1 && (
                            <Animated.View entering={FadeInUp.duration(300).delay(30)} style={{ marginTop: 16 }}>
                                <ScrollView
                                    horizontal
                                    ...
                                    {orderLocations.map((location) => {
                                        ...
                                        return (
                                            <Pressable
                                                ...
                                                style={[styles.locationTab, isSelected && styles.locationTabActive]}
                                            >
                                                <Text style={[styles.locationTabText, isSelected && styles.locationTabTextActive]}>
                                                    {location.name}
                                                </Text>
                                            </Pressable>
                                        );
                        ...
                        {/* Add card */}
                        <Animated.View entering={FadeInUp.duration(300).delay(50)} style={styles.addCard}>
                            <BrText variant="eyebrow" style={{ marginBottom: 8 }}>Add an item</BrText>

                            {/* Input + add button */}
                            <View style={styles.inputRow}>
                                <TextInput
                                    value={itemInput}
                                    onChangeText={(text) => { setItemInput(text); if (saveError) setSaveError(null); }}
                            ...
                            <View style={styles.orRow}>
                                ...
                                <BrText variant="eyebrow" style={{ paddingHorizontal: 10, color: BR.ink3 }}>or</BrText>
                                <View style={styles.orLine} />
                            </View>
                            ...
                            <TouchableOpacity
                                ...
                                style={[styles.dictateBtn, (isSaving || isCompletingOrder || isProcessingVoice) && { opacity: 0.5 }]}
                            >
                            ...
                            <View style={styles.hintRow}>
                                ...
                                <Text style={styles.hintText}>One line per item — keep it scannable for the runner.</Text>
                            </View>
                        </Animated.View>
                        ...
                        <Animated.View entering={FadeInUp.duration(300).delay(80)} style={{ marginTop: 22 }}>
                            <View style={styles.listHeader}>
                                <BrText variant="eyebrow">Your list · {selectedLocationName}</BrText>
                                {currentItems.length > 0 && (
                                    <Pressable onPress={() => { setCurrentItems([]); setSaveError(null); }} hitSlop={8}>
                                ...
                                <View style={styles.receiptOuter}>
                                    ...
                                    <View style={styles.emptyCardInner}>
                                        <View style={styles.emptyIconWrap}>
                                            <Icon name="UtensilsCrossed" size={28} color={BR.orangeDeep} />
                                        </View>
                                        <Text style={styles.emptyTitle}>Nothing on the list yet</Text>
                                        <Text style={styles.emptyMono}>· TYPE OR DICTATE TO BEGIN ·</Text>
                                        <View style={styles.receiptDivider} />
                                        <Text style={styles.emptyHint}>
                                            Add items above or tap "Dictate your order" to use voice AI.
                                        </Text>
                                    </View>
                                ...
                                <View style={styles.receiptOuter}>
                                    ...
                                    <View style={styles.receiptInner}>
                                        <View style={{ alignItems: "center", paddingBottom: 12 }}>
                                            <Text style={styles.receiptStoreName}>{selectedLocationName}</Text>
                                            <Text style={styles.receiptSubtitle}>
                                                · {totalItems} ITEM{totalItems === 1 ? "" : "S"} · RUNNER WILL GRAB ·
                                            </Text>
                                        </View>
                                        <View style={styles.receiptDivider} />
                                        ...
                                        <View style={[styles.receiptDivider, { marginTop: 14, marginBottom: 6 }]} />
                                        <View style={styles.subtotalRow}>
                                            <Text style={styles.subtotalLabel}>SUBTOTAL · ITEMS</Text>
                ...
                <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
                    <TouchableOpacity
                        ...
                        style={[styles.doneBtn, totalItems === 0 && styles.doneBtnEmpty, isCompletingOrder && { opacity: 0.6 }]}
                    >
                        {isCompletingOrder ? (
                            <ActivityIndicator size="small" color="#fff" />
                        ) : (
                            <Icon name="Check" size={18} color={totalItems === 0 ? BR.ink2 : "#fff"} />
                        )}
                        <Text style={[styles.doneBtnText, totalItems === 0 && { color: BR.ink2 }]}>
                            {isCompletingOrder
                                ? "Finishing..."
                                : totalItems === 0
            ...
            >
                <View style={voiceStyles.screen}>
                    ...
                    {/* Header */}
                    <View style={voiceStyles.vHeader}>
                        <Pressable
                    ...
                    {parsedItems.length === 0 ? (
                        ...
                        <View style={voiceStyles.listeningPhase}>
                            <VoiceRippleOrb isListening={isListening} isProcessingVoice={isProcessingVoice} />
                            ...
                            <View style={voiceStyles.vTranscriptWrap}>
                                {displayedVoiceTranscript ? (
                                    <Text style={voiceStyles.vTranscriptText}>
                                        {`"${displayedVoiceTranscript}`}
                                        {isListening && (
                                            <Text style={{ opacity: cursorVisible ? 1 : 0, color: BR.orange }}>|</Text>
                                        )}
                                        {`"`}
                                    </Text>
                                ) : (
                            ...
                            <View style={voiceStyles.vBottomRow}>
                                {isListening ? (
                                    ...
                                    >
                                        <Icon name="Square" size={14} color="#fff" strokeWidth={2} />
                                        <Text style={voiceStyles.vActionBtnText}>Tap to stop</Text>
                                    </TouchableOpacity>
                                ) : isProcessingVoice ? (
                                    <View style={{ alignItems: "center", gap: 10 }}>
                                        <Flow size={28} color={BR.orange} />
                                        <Text style={voiceStyles.vProcessingText}>AI is parsing your order</Text>
                                    </View>
                                ) : (
                                    <TouchableOpacity
                                        onPress={() => void startVoiceOrdering()}
                                ...
                                {!isListening && !isProcessingVoice && !voiceError && (
                                    <Text style={voiceStyles.vHintText}>Speak naturally · one item at a time</Text>
                                )}
                            </View>
                        </View>
                    ) : (
                        /* ── Parsed phase ── */
                        <Animated.View entering={FadeInUp.duration(350)} style={voiceStyles.parsedPhase}>
                                ...
                                {parsedItems.map((item, i) => (
                                    ...
                                    >
                                        <View style={voiceStyles.parsedItemBadge}>
                                            <Text style={voiceStyles.parsedItemBadgeText}>{String(i + 1).padStart(2, "0")}</Text>
                                        </View>
                            ...
                            <View style={voiceStyles.parsedButtons}>
                                <TouchableOpacity onPress={confirmVoiceItems} style={voiceStyles.confirmBtn}>
                                    <Icon name="Check" size={16} color="#fff" strokeWidth={2.5} />
                                    <Text style={voiceStyles.confirmBtnText}>
                                        Add {parsedItems.length} {parsedItems.length === 1 ? "item" : "items"}
                                    </Text>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => void resetVoice()} style={voiceStyles.tryAgainBtn}>
