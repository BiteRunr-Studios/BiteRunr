...
function SummaryShell({ title, children }: { title: string; children: ReactNode }) {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: BR.paper, paddingTop: insets.top }}>
            <View style={styles.header}>
                ...
                <BrText weight="bold" style={{ fontSize: 17 }}>{title}</BrText>
                <View style={{ width: 38 }} />
            </View>
            {children}
        </View>
        ...
        <SummaryShell title="Order Summary">
            <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 24 }}>
                <View style={styles.loadingCard}>
                    ...
                    <BrText weight="bold" style={{ fontSize: 22, marginTop: 16, textAlign: "center" }}>
                        Summarizing order
                    </BrText>
        ...
        <SummaryShell title="Order Summary">
            <Skeleton>
                <View style={{ padding: 18, gap: 14 }}>
                    ...
                    {[1, 2, 3].map((i) => (
                        <SkeletonBlock key={i} width="100%" height={64} rounded="rounded-2xl" />
                    ))}
                </View>
            </Skeleton>
        </SummaryShell>
...
function ScanningOverlay({
    ...
    return (
        <Modal visible animationType="fade" presentationStyle="fullScreen" statusBarTranslucent>
            <View style={[scanStyles.root, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 16 }]}>

                {/* Status pill */}
                <View style={[scanStyles.statusPill, { backgroundColor: isError ? BR.coral : BR.orange }]}>
                    {isError
                    ...
                    <Text style={scanStyles.statusPillText}>
                        {isError ? "Scan failed" : isUploading ? "Uploading…" : "AI is reading…"}
                    </Text>
                </View>
                ...
                >
                    ...
                    <View style={[scanStyles.photoInner, isError && { opacity: 0.5 }]}>
                        {photoUri ? (
                            ...
                            <View style={[StyleSheet.absoluteFill, { alignItems: "center", justifyContent: "center" }]}>
                                <Icon name="ScanLine" size={48} color="rgba(255,255,255,0.15)" />
                            </View>
                        )}
                        {/* Subtle dark overlay */}
                    ...
                    {/* Scanning beam */}
                    {!isUploading && !isError && (
                        <Animated.View style={[scanStyles.beamWrapper, beamStyle]} pointerEvents="none">
                    ...
                    {isError ? (
                        <>
                            ...
                            <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
                                <TouchableOpacity
                                    onPress={onRetry}
                                    style={[scanStyles.actionBtn, { backgroundColor: BR.orange, flex: 1 }]}
                                    activeOpacity={0.85}
                                >
                                ...
                                <TouchableOpacity
                                    onPress={onDismiss}
                                    style={[scanStyles.actionBtn, { backgroundColor: "rgba(255,255,255,0.1)", flex: 1 }]}
                                    activeOpacity={0.85}
                                >
                                    <Text style={[scanStyles.actionBtnText, { color: "rgba(255,255,255,0.6)" }]}>Cancel</Text>
                                </TouchableOpacity>
                            </View>
                        </>
                    ) : (
...
export default function OrderSummary() {
    ...
    const aiHint: SummaryAiHint =
        params.aiHint === "cached" ? "cached"
        : params.aiHint === "generate" ? "generate"
        : null;
    ...
    useEffect(() => {
        ...
        Toast.show({ type: "success", text1: "Receipt prices saved", visibilityTime: 2500 });
    ...
    useEffect(() => {
        ...
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        Toast.show({ type: "success", text1: "Prices saved", visibilityTime: 2500 });
    ...
    const currentAiLocationSummary = useMemo(
        () => aiSummary?.locations.find((l) => l.orderLocationId === selectedLocation?.orderLocationId) ?? null,
        [aiSummary, selectedLocation?.orderLocationId],
    );
    const itemGroups = useMemo(
        () => materializeResolvedOrderItemTextGroups(currentLocationLines, currentAiLocationSummary?.groups),
        [currentAiLocationSummary?.groups, currentLocationLines],
    );
    const isUsingLocalSummaryFallback = aiSummaryStatus === "ai-error";

    if (summary === undefined) {
        return aiHint === "generate" ? <SummaryAiLoadingScreen /> : <SummaryDataLoadingScreen />;
    }
    if (summary === null) {
        return (
            <View style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: BR.paper }}>
                <BrText style={{ color: BR.coralInk }}>Not authorized to view this summary</BrText>
            </View>
        );
    }
    ...
    if (!aiSummary && !isUsingLocalSummaryFallback) return <SummaryDataLoadingScreen />;

    const isScanning = scanState === "uploading" || scanState === "parsing";
    const allLocationsPriced = summary.locationSummaries.every((ls) => ls.subtotalInCents !== null);
    const locationsWithPrices = summary.locationSummaries.filter((ls) => ls.subtotalInCents !== null);
    const locationsMissingPrices = summary.locationSummaries.filter((ls) => ls.subtotalInCents === null);
    const someLocationsPriced = locationsWithPrices.length > 0 && locationsMissingPrices.length > 0;
        ...
        <>
            {(isScanning || scanState === "error") && (
                <ScanningOverlay
                    ...
                    scanState={scanState}
                    error={scanError}
                    onRetry={() => { resetScan(); sourceActionSheetRef.current?.show(); }}
                    onDismiss={resetScan}
                />
            )}
            ...
            <View style={{ flex: 1, backgroundColor: BR.paper }}>
                ...
                <View style={styles.header}>
                    ...
                    <BrText weight="bold" style={{ fontSize: 17, flex: 1, marginHorizontal: 8 }} numberOfLines={1}>
                ...
                <View style={{ paddingHorizontal: 18, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: BR.line }}>
                    <Text style={styles.statsLine}>
                        {summary.totalPeople} {summary.totalPeople === 1 ? "person" : "people"} · {summary.totalItems} {summary.totalItems === 1 ? "item" : "items"}
                    </Text>
                    <ScrollView
                        horizontal
                    ...
                    >
                        {summary.locations.map((location: SummaryLocation) => {
                            ...
                            const isSelected = selectedLocation?.orderLocationId === location.orderLocationId;
                            const hasPrices = locationSummary?.subtotalInCents !== null;
                            return (
                                <Pressable
                                    key={location.orderLocationId}
                                    onPress={() => setSelectedLocationId(location.orderLocationId)}
                                    style={[styles.locationChip, isSelected && styles.locationChipActive]}
                                >
                                    {hasPrices && (
                                        <Icon name="CircleCheck" size={13} color={isSelected ? "#fff" : BR.mint} />
                                    )}
                ...
                {manualState === "error" && manualError && (
                    <View style={[styles.banner, { backgroundColor: BR.coralSoft }]}>
                        ...
                        <Text style={[styles.bannerText, { color: BR.coralInk, flex: 1 }]}>{manualError}</Text>
                ...
                {isUsingLocalSummaryFallback && (
                    <View style={[styles.banner, { backgroundColor: BR.yolkSoft }]}>
                        ...
                        <Text style={[styles.bannerText, { color: "#7A4A20", flex: 1 }]}>
                            {aiSummaryError ?? "AI grouping is unavailable. Showing a local summary."}
                        </Text>
                        <Pressable
                            onPress={retryAiSummary}
                            style={{ paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999, backgroundColor: BR.yolk }}
                        >
                    ...
                    <Pressable
                        onPress={() => {
                            const nextUnpriced = summary.locations.find((location) =>
                                locationsMissingPrices.some((ls) => ls.orderLocationId === location.orderLocationId),
                            );
                            if (nextUnpriced) setSelectedLocationId(nextUnpriced.orderLocationId);
                        }}
                        ...
                        <>
                            <View style={styles.sectionHeader}>
                                ...
                                <View style={styles.countBadge}>
                                    <Text style={styles.countBadgeText}>{currentLocationSummary.itemCount}</Text>
                                </View>
                            </View>

                            {groupMode === "items"
                                ? itemGroups.map((group, i) => (
                                    <Animated.View key={group.key} entering={FadeInUp.duration(200).delay(i * 30)}>
                                        <View style={styles.itemCard}>
                                            <Text style={styles.itemName}>{group.displayName}</Text>
                                            <View style={styles.itemCountBadge}>
                                                <Text style={styles.itemCountText}>{group.lineCount}</Text>
                                            </View>
                                        </View>
                                    </Animated.View>
                                ))
                                : participantGroups.map((group, gi) => (
                                    <Animated.View key={group.key} entering={FadeInUp.duration(200).delay(gi * 30)}>
                                        <View style={styles.personCard}>
                                            <Text style={styles.personName}>{group.baseName}</Text>
                                            <View style={{ gap: 6, marginTop: 10 }}>
                                                {group.items.map((line) => (
                                                    <View key={line.id} style={styles.personItemRow}>
                                                        <Text style={styles.personItemText}>{line.text}</Text>
                            ...
                            {currentLocationSummary.subtotalInCents !== null && (
                                <>
                                    <Text style={[styles.sectionLabel, { marginTop: 8 }]}>Price summary</Text>
                ...
                <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) + 8 }]}>
                    {isScanning ? (
                        <View style={[styles.footerBtn, { backgroundColor: BR.orangeSoft, borderWidth: 1, borderColor: "rgba(255,106,31,0.25)" }]}>
                            <ActivityIndicator size="small" color={BR.orange} />
                            <Text style={[styles.footerBtnText, { color: BR.orangeDeep }]}>
                                {scanState === "uploading" ? "Sending your photo…" : "Reading your receipt…"}
                            </Text>
                        </View>
                    ) : hasScanDraft ? (
                        <TouchableOpacity
                        ...
                        <View style={{ gap: 10 }}>
                            <TouchableOpacity
                                onPress={() => router.push(`/order/settlement?orderId=${orderId}` as never)}
                            ...
                            >
                                <Text style={{ fontSize: 13, fontWeight: "600", color: BR.orangeDeep }}>Edit prices</Text>
                            </TouchableOpacity>
                        </View>
                    ) : (
                        <TouchableOpacity
            ...
            >
                <View style={{ paddingHorizontal: 18, paddingTop: 8 }}>
                    <BrText weight="bold" style={{ fontSize: 20, textAlign: "center" }}>Add prices</BrText>
                    <Text style={{ fontSize: 13, color: BR.ink3, textAlign: "center", marginTop: 4, marginBottom: 18 }}>
                        Scan a receipt or enter prices by hand
                    </Text>

                    {[
                        { icon: "Camera" as const, title: "Take photo", sub: "Use your camera to capture the receipt", onPress: () => { sourceActionSheetRef.current?.hide(); void startScan("camera"); } },
                        { icon: "Image" as const, title: "Choose from library", sub: "Select an existing photo", onPress: () => { sourceActionSheetRef.current?.hide(); void startScan("library"); } },
                        { icon: "DollarSign" as const, title: "Enter manually", sub: "Type in prices for each order line", onPress: () => { sourceActionSheetRef.current?.hide(); setTimeout(() => startManualEntry(), 400); } },
                    ].map((item) => (
                        <TouchableOpacity
                            key={item.title}
                    ...
                    <TouchableOpacity
                        ...
                        style={[styles.footerBtn, { marginTop: 8, backgroundColor: BR.paper2, borderWidth: 1, borderColor: BR.line2 }]}
                        activeOpacity={0.85}
                    >
                        <Text style={[styles.footerBtnText, { color: BR.ink }]}>Cancel</Text>
                    </TouchableOpacity>
                </View>
            </ActionSheet>
