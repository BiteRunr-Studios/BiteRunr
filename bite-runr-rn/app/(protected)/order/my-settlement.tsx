...
function getCurrentStep(
    ...
    if (settlementStatus === "confirmed" || settlementStatus === "settled_in_person") return 2;
...
export default function MySettlement() {
    ...
    const createPaymentSheetParams = useAction(api.stripeConnect.createPaymentSheetParams);

    const handlePayWithCard = async () => {
        if (!orderId) return;
        setIsPaying(true);
        try {
            const sheetParams = await createPaymentSheetParams({ orderId: orderId as Id<"orders"> });
            ...
            Alert.alert("Error", error instanceof Error ? error.message : "Failed to create payment");
        } finally {
            setIsPaying(false);
        }
    };
    ...
    const currentStep = getCurrentStep(settlement.settlementStatus, settlement.stripePayment);
    const isSettled =
        ...
        <>
            ...
            >
                                ...
                                {showFeeBreakdown ? (
                                    <>
                                        ...
                                        <View style={styles.receiptRow}>
                                            ...
                                            <Text style={styles.rowValue}>{formatCents(serviceFee)}</Text>
                                        </View>
                                    </>
                                ) : null}
                ...
                <Animated.View entering={FadeInUp.duration(300).delay(60)} style={{ marginTop: 22 }}>
                    <BrText variant="eyebrow" style={{ marginBottom: 10 }}>Payment status</BrText>
            ...
            {canPay && (
                <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
                    ...
                    <TouchableOpacity
                        ...
                        style={[
                            styles.ghostBtn,
                            !runnerAcceptsCards && styles.primaryBtn,
                        ]}
                    >
                        <Icon
                            name="Banknote"
