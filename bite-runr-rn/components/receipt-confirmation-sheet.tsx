...
interface ReceiptConfirmationSheetProps {
    ...
    onUpdateMatch: (index: number, orderItemId: string | null, priceInCents?: number | null) => void;
...
function confidenceConfig(confidence: number) {
    if (confidence >= 0.7) return { label: "Great", bg: BR.mintSoft, fg: BR.mintInk };
    if (confidence >= 0.4) return { label: "Maybe", bg: BR.yolkSoft, fg: "#7A4A20" };
    ...
    return (
        <Pressable
            ...
            style={[
                ...
                isSelected && styles.unmatchedCardSelected,
            ]}
        >
            <View style={[styles.unmatchedIconCircle, isSelected && { backgroundColor: BR.orangeSoft }]}>
                <Icon
            ...
            <View style={{ flex: 1 }}>
                ...
                {item.receiptItem.comboName && (
                    <Text style={styles.unmatchedCombo}>Split from {item.receiptItem.comboName}</Text>
                )}
            </View>
            {isSelected && (
                <Text style={styles.unmatchedHint}>Tap item below</Text>
            )}
        </Pressable>
    );
}
...
function OrderItemCard({
    ...
    const handlePriceChange = (text: string) => {
        ...
        if (!isNaN(value)) onUpdatePrice(matchedIndex, Math.round(value * 100));
        else if (cleaned === "") onUpdatePrice(matchedIndex, null);
    };

    return (
        <Pressable
            onPress={() => { if (isLinkable) onLinkSelected(orderItem.id); }}
            style={[
        ...
        >
            ...
            <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 10 }}>
                ...
                <View style={{ flex: 1 }}>
                    ...
                    {!linkedReceiptItem && (
                        <Text style={[styles.orderItemHint, isLinkable && { color: BR.orangeDeep }]}>
                            {isLinkable ? "Tap to link here" : "No receipt item linked"}
                        </Text>
                    )}
                </View>
                {linkedReceiptItem && effectivePrice !== null && (
                    <Text style={styles.orderItemPrice}>{formatPrice(effectivePrice)}</Text>
                )}
            </View>
                        ...
                        {linkedReceiptItem.confidence < 1 && (() => {
                            ...
                            return (
                                <View style={[styles.confidenceBadge, { backgroundColor: cfg.bg }]}>
                                    <Text style={[styles.confidenceText, { color: cfg.fg }]}>{cfg.label}</Text>
                                </View>
                            );
                        })()}
                    </View>
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                        <View style={styles.priceInputRow}>
                            ...
                            <TextInput
                                ...
                                placeholder={effectivePrice !== null ? (effectivePrice / 100).toFixed(2) : "0.00"}
...
function PersonSection({
    name, matchedCount, totalCount, allMatched, collapsed, onToggle, children,
}: {
    name: string; matchedCount: number; totalCount: number;
    allMatched: boolean; collapsed: boolean; onToggle: () => void;
    children: React.ReactNode;
}) {
    const [rotation] = useState(() => new Animated.Value(collapsed ? 1 : 0));
    useEffect(() => {
        Animated.timing(rotation, { toValue: collapsed ? 1 : 0, duration: 200, useNativeDriver: true }).start();
    }, [collapsed, rotation]);
...
export function ReceiptConfirmationSheet({
    visible, onClose, onConfirm, matchedItems, orderItems,
    onUpdateMatch, receiptStoreName, receiptTotal, isSaving,
}: ReceiptConfirmationSheetProps) {
    const insets = useSafeAreaInsets();
    const [selectedReceiptIndex, setSelectedReceiptIndex] = useState<number | null>(null);
    const [sectionCollapseOverrides, setSectionCollapseOverrides] = useState<Map<string, boolean>>(new Map());

    const personGroups = useMemo(() => groupOrderItemsByParticipant(orderItems), [orderItems]);

    const matchByOrderItemId = useMemo(() => {
        const map = new Map<string, { item: MatchedItem; index: number }>();
        matchedItems.forEach((m, i) => { if (m.matchedOrderItemId) map.set(m.matchedOrderItemId, { item: m, index: i }); });
        return map;
    }, [matchedItems]);

    const toggleSection = useCallback((participantKey: string, defaultCollapsed: boolean) => {
    ...
    const unmatchedItems = useMemo(
        () => matchedItems.map((item, index) => ({ item, index })).filter(({ item }) => !item.matchedOrderItemId),
        [matchedItems],
    );

    const totalOrderItems = orderItems?.length ?? 0;
    const pricedOrderItemCount = orderItems
        ? orderItems.filter((oi) => getEffectivePrice(matchByOrderItemId.get(oi.id)?.item) !== null).length
        : 0;
    const comboSplitCount = matchedItems.filter((m) => !!m.receiptItem.comboName).length;
    const allOrderItemsPriced = totalOrderItems > 0 && pricedOrderItemCount === totalOrderItems;
        ...
        <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" statusBarTranslucent onRequestClose={onClose}>
            <View style={[styles.root, { paddingTop: insets.top }]}>
                ...
                <View style={styles.modalHeader}>
                    <View style={{ flex: 1 }}>
                        <BrText weight="bold" style={{ fontSize: 20 }}>Review items</BrText>
                        {receiptStoreName && (
                            <Text style={{ fontSize: 12, color: BR.ink3, marginTop: 2, fontFamily: BR_FONT.mono }}>{receiptStoreName}</Text>
                        )}
                    </View>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                        {receiptTotal !== null && (
                            <View style={styles.receiptTotalBadge}>
                                <Text style={styles.receiptTotalText}>Total: {formatPrice(receiptTotal)}</Text>
                            </View>
                        )}
                ...
                {hasSelectedItem && (
                    <View style={styles.linkingBanner}>
                        ...
                        <TouchableOpacity onPress={() => setSelectedReceiptIndex(null)} style={styles.linkingCancelBtn}>
                ...
                {comboSplitCount > 0 && (
                    <View style={styles.comboInfoCard}>
                        ...
                        <View style={{ flex: 1 }}>
                            <Text style={styles.comboInfoTitle}>Combo items were split out</Text>
                            <Text style={styles.comboInfoBody}>
                                Included items were separated so you can match them one by one. Prices start as an even split.
                            </Text>
                        </View>
                    </View>
                )}
                ...
                >
                    {unmatchedItems.length > 0 && (
                        <>
                            <SectionHeader
                                title={allOrderItemsPriced ? "Unlinked receipt lines" : "Needs attention"}
                    ...
                    {allOrderItemsPriced && unmatchedItems.length === 0 && personGroups.length > 0 && (
                        <View style={styles.allMatchedBanner}>
                            <Icon name="CircleCheck" size={26} color={BR.mint} />
                            <Text style={styles.allMatchedText}>All order items priced!</Text>
                        </View>
                    )}
                </ScrollView>

                {/* Footer */}
                <View style={[styles.modalFooter, { paddingBottom: Math.max(insets.bottom, 16) + 4 }]}>
                    ...
                    <TouchableOpacity
                        ...
                        activeOpacity={0.85}
                        style={[styles.saveBtn, (isSaving || pricedOrderItemCount === 0) && { opacity: 0.55 }]}
                    >
                        {isSaving
