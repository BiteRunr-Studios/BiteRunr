...
function buildParticipantGroups<T extends GroupableOrderItem>(
  ...
  for (const [baseName, duplicateGroups] of groupsByBaseName.entries()) {
    ...
    [...duplicateGroups]
      ...
      .forEach((group, index) => {
        displayNameByParticipantId.set(group.key, `${baseName} (${index + 1})`);
      });
  }
  ...
  return groupsInDisplayOrder.map((group) => ({
    ...
    displayName: displayNameByParticipantId.get(group.key) ?? group.baseName,
    items: group.items,
  }));
}
...
function buildTextGroup<T extends ItemTextGroupableOrderItem>(
  ...
  return {
    ...
    subtotalInCents: allItemsPriced
      ? sortedItems.reduce((sum, item) => sum + (item.priceInCents ?? 0), 0)
      : null,
  };
}
...
function buildAmbiguousClusters<T extends ItemTextGroupableOrderItem>(
  ...
  for (let index = 0; index < groups.length; index += 1) {
    for (
      ...
      if (!tokenSetsOverlap(leftMetadata.tokenSet, rightMetadata.tokenSet)) {
        continue;
      }
...
export function buildOrderSummaryTextSignature<
  T extends Pick<ItemTextGroupableOrderItem, "id" | "text">,
>(
  locations: OrderSummaryTextSignatureLocation<T>[] | null | undefined,
): string {
  return [...(locations ?? [])]
    .sort((left, right) =>
...
export function analyzeOrderItemTextGroups<
  T extends ItemTextGroupableOrderItem,
>(orderItems: T[] | null | undefined): OrderItemTextGroupingAnalysis<T> {
...
export function groupOrderItemsByItemText<T extends ItemTextGroupableOrderItem>(
  orderItems: T[] | null | undefined,
): OrderItemTextGroup<T>[] {
