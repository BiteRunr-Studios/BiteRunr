...
export const getSettlementCheckoutData = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const buyerOrderUser = orderUsers.find((ou) => ou.userId === args.buyerId);
    ...
    if (!sellerAccount?.chargesEnabled) return null;
...
export const getMySettlementStatus = query({
  ...
  handler: async (ctx, args) => {
    ...
    const latestPayment =
      payments.length > 0
        ? payments.reduce((a, b) => (a._creationTime > b._creationTime ? a : b))
        : null;
