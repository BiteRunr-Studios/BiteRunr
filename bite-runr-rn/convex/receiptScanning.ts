...
function allocatePerUnitPrices(
  ...
  if (componentQuantities.length > MAX_INCLUDED_ITEMS) {
    ...
    let remainingRemainder = comboUnitPriceInCents - baseUnitPrice * totalUnits;
...
function normalizeParsedReceiptItems(
  ...
  for (let index = 0; index < parsedItems.length; index++) {
    ...
    for (
      ...
      if (
        remainingIncludedItems.every(
          (includedItem) => includedItem.remainingQuantity <= 0,
        )
      ) {
        break;
      }
    ...
    const splitUnitPrices =
      item.priceInCents !== null
        ? allocatePerUnitPrices(item.priceInCents, totalIncludedQuantities)
    ...
    includedItems.forEach((includedItem, includedIndex) => {
      ...
      normalizedItems.push({
        ...
        priceInCents: unitPrice !== null ? unitPrice * totalQuantity : null,
        comboName: itemName,
        comboItems,
  ...
  handler: async (
    ...
    if (!imageUrl) {
      ...
      return {
        success: false,
        error:
          "We couldn't process your photo. Please try taking or selecting the image again.",
      };
    }
    ...
    try {
      ...
      const result = await generateObject({
        model: openrouter.chat("qwen/qwen3.5-35b-a3b", {
          // options: google/gemini-2.5-flash-lite / qwen/qwen3.5-35b-a3b
          provider: {
            order: ["parasail/fp8", "alibaba"], // qwen: "parasail/fp8", "alibaba" / google: "google-vertex/eu", "google-ai-studio"
      ...
      if (parsed.status === "not_a_receipt") {
        return {
          success: false,
          error:
            "This doesn't look like a receipt. Please take or select a photo of your receipt and try again.",
        };
      }
      ...
      if (parsed.status === "unreadable_receipt") {
        return {
          success: false,
          error:
            "The receipt is too blurry or dark to read. Please take a clearer photo and try again.",
        };
      }
      ...
      if (parsed.items.length === 0) {
        return {
          success: false,
          error:
            "No items found on the receipt. Please make sure the full receipt is visible in the photo.",
        };
      }
      ...
      return {
        success: false,
        error:
          "Something went wrong while scanning your receipt. Please try again.",
      };
    } finally {
    ...
    for (const orderUser of orderUsers) {
      ...
      for (const item of userItems) {
        if (item.priceInCents !== undefined) {
          ...
          const prev = subtotalByLocation.get(item.orderLocationId) ?? 0;
          subtotalByLocation.set(item.orderLocationId, prev + itemTotal);
        }
      }
