...
export const orderItemsCount: AppRouteHandler<OrderItemsCountRoute> = async (
  ...
  for (const order_location of order_locations) {
    const order_location_items = await db.query.orderItems.findMany({
      where(fields, operators) {
        return operators.eq(fields.order_location_id, order_location.id);
      },
    });
...
> = async (c) => {
  ...
  return c.json(order_locations_with_location_name_parsed, HttpStatusCodes.OK);
};
...
> = async (c) => {
  ...
  const [updatedOrder] = await db
    ...
    .set({ status })
    .where(
      and(eq(orderUsers.order_id, order_id), eq(orderUsers.user_id, user_id))
    )
    .returning();
